...
botao.addEventListener("click", function (mostraMsg) {
  mostraMsg = prompt(
    "Você deve tomar decisões sobre a situação que o personagem escolhido se encontra, informando a sua opção de escolha através dos prompts na tela.\nSerá que nossos laxcados se sairão bem? \nVocê quer jogar?\nDigite sim ou não"
  );
  if (mostraMsg.toLowerCase() == "sim") {
    return (location = "./html/escolhaDePersonagem.html");
  } else if (
    mostraMsg.toLowerCase() == "não" ||
    mostraMsg.toLowerCase() == "nao"
  ) {
